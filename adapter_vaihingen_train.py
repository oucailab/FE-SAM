...
def inference(config, model, valid_dataloaders, multimask_output):
        ...
        for batch in iterator:
            ...
            for i in range(images.size(0)):
                ...
                try:
                    ...
                    predictions = torch.nn.Softmax(dim=1)(predictions["masks"])
    ...
    write_to_log(f"{config.output_path}/train_logs.txt", data)
    return np.nanmean(f1_per_class[:-1]), np.nanmean(iou_per_class[:-1]), OA


if __name__ == "__main__":
    os.environ['ALBUMENTATIONS_DISABLE_VERSION_CHECK'] = '1'

    config = py2cfg("/root/autodl-tmp/SAM-Adapter-main/configs/adapter_sam_vh.py")
