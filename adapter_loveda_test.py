...
def label2rgb_loveda(mask):
    h, w = mask.shape[0], mask.shape[1]
    mask_rgb = np.zeros(shape=(h, w, 3), dtype=np.uint8)
    mask_convert = mask[np.newaxis, :, :]

    mask_rgb[np.all(mask_convert == 0, axis=0)] = [255, 255, 255]  # background - white
    mask_rgb[np.all(mask_convert == 1, axis=0)] = [255, 0, 0]     # building - red
    mask_rgb[np.all(mask_convert == 2, axis=0)] = [255, 255, 0]   # road - yellow
    mask_rgb[np.all(mask_convert == 3, axis=0)] = [0, 0, 255]     # water - blue
    mask_rgb[np.all(mask_convert == 4, axis=0)] = [159, 129, 183] # barren - purple
    mask_rgb[np.all(mask_convert == 5, axis=0)] = [0, 255, 0]     # forest - green
    mask_rgb[np.all(mask_convert == 6, axis=0)] = [255, 195, 128] # agricultural - orange
    mask_rgb[np.all(mask_convert == 7, axis=0)] = [0, 0, 0]       # invalid - black
...
def img_writer_rgb(inp):
    ...
    if rgb:
        mask_name_png = mask_id + '_rgb.png'
        mask_rgb = label2rgb_loveda(mask)
...
def extract_original_filename_from_batch(batch, batch_idx, sample_idx, dataset):
    try:
        ...
        absolute_idx = batch_idx * len(batch['image']) + sample_idx
        return f"loveda_rural_{absolute_idx:06d}"


def test_loveda_model(config, model, test_dataloaders, multimask_output, output_path, tta_type=None, rgb=True):
    model.eval()
    ...
    print(f"Testing LoveDA Rural dataset on {len(test_dataloaders.dataset)} samples...")
    iterator = tqdm(test_dataloaders, desc="Testing LoveDA")

    results = []
    total_processed = 0
    successful_saves = 0

    label_stats = np.zeros(config.num_classes)
    pred_stats = np.zeros(config.num_classes)
            ...
            for i in range(images.size(0)):
                ...
                if total_processed % 100 == 0:
                    ...
                    if single_label is not None:
                        print(f"Label shape: {single_label.shape}")
                        unique_labels = torch.unique(single_label)
                        print(f"Unique labels in this sample: {unique_labels.cpu().numpy().tolist()}")
                ...
                try:
                    ...
                    unique_pred, counts_pred = np.unique(mask, return_counts=True)
                    for label_val, count in zip(unique_pred, counts_pred):
                        if int(label_val) < config.num_classes:
                            pred_stats[int(label_val)] += count

                    if single_label is not None:
                        label = single_label[0].cpu().numpy()

                        unique_gt, counts_gt = np.unique(label, return_counts=True)
                        for label_val, count in zip(unique_gt, counts_gt):
                            if int(label_val) < config.num_classes:
                                label_stats[int(label_val)] += count
    ...
    for i, (class_name, count) in enumerate(zip(config.classes, label_stats)):
        percentage = (count / label_stats.sum()) * 100 if label_stats.sum() > 0 else 0
        print(f"  {class_name}: {count:,} pixels ({percentage:.2f}%)")

    print("\nPrediction Distribution:")
    for i, (class_name, count) in enumerate(zip(config.classes, pred_stats)):
        percentage = (count / pred_stats.sum()) * 100 if pred_stats.sum() > 0 else 0
        print(f"  {class_name}: {count:,} pixels ({percentage:.2f}%)")
    ...
    if hasattr(evaluator, 'confusion_matrix') and evaluator.confusion_matrix.sum() > 0:
        ...
        print("\n=== LoveDA Rural Evaluation Results ===")
        print("Per-class results (including invalid class):")
        ...
        print(f"\nFinal results (excluding invalid class, following GeoSeg approach):")
        ...
        with open(results_file, 'w') as f:
            f.write("=== LoveDA Rural Evaluation Results ===\n")
            f.write("Per-class results:\n")
            for class_name, class_iou, class_f1 in zip(config.classes, iou_per_class, f1_per_class):
                f.write(f'F1_{class_name}:{class_f1:.4f}, IOU_{class_name}:{class_iou:.4f}\n')
            f.write(f"\nFinal results (excluding invalid class):\n")
            f.write(f'F1: {final_f1:.4f}\n')
...
def get_args():
    parser = argparse.ArgumentParser(description='SAM-Adapter LoveDA Rural Model Testing')
...
def main():
    ...
    print("=== SAM-Adapter LoveDA Rural Model Testing ===")
    ...
    net = pkg.FESAM(sam, num_classes=config.num_classes).to(config.device)
    ...
    print("================> Start LoveDA Rural testing <================")
    num_results = test_loveda_model(
        config, net, test_dataloaders, multimask_output,
        args.output_path, args.tta, args.rgb
    )
    print(f"LoveDA Rural testing completed! Processed {num_results} samples.")
